...
def build_portfolio(scores: pd.Series, top_k: int = DEFAULT_TOP_K, score_weighted: bool = False) -> pd.Series:
    """Top-K names, weight proportional to (rank or score) then capped at MAX_WEIGHT.

    score_weighted=True: weights proportional to predicted score (shifted to be non-negative).
    score_weighted=False (default): rank-based weights.
    After capping at 10% we redistribute spillover to uncapped names and iterate until feasible.
    """
    if top_k < MIN_STOCKS:
    ...
    if score_weighted:
        # Shift so minimum = small positive floor, then normalize.
        shifted = chosen - chosen.min() + 1e-6
...
def main():
    ...
    p.add_argument("--score-weighted", action="store_true", help="use score-weighted instead of rank-weighted portfolio")
    ...
    weights = build_portfolio(scores, top_k=args.top_k, score_weighted=args.score_weighted)
