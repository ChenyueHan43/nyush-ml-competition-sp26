...
\textbf{Rank-weighted top-30.} Stocks are ranked by predicted score; the
top-30 receive initial weights proportional to their rank:
\[
  \tilde{w}_i = \frac{\text{rank}_i}{\sum_{j=1}^{30} \text{rank}_j},
  \quad \text{rank}_i \in \{1, 2, \ldots, 30\}
  \quad \text{(rank 30 = highest score)}
\]
Weights are linear in rank, so the top stock receives weight $30/465
\approx 6.5\%$ and the bottom stock receives $1/465 \approx 0.2\%$.
...
\textbf{Why top-30?} A walk-forward backtest on the \emph{validation set}
(2024-11-01 to 2025-05-31, $n=27$ windows) compared top-$k \in
\{30, 40, 50\}$. Top-30 achieved the highest cumulative excess return
($+50.45\%$) and was selected based solely on validation performance.
The test set was not consulted for this decision.
...
\midrule
XGBoost baseline --- top-50 & $+23.18\%$ & 59\% & 0.38 & $-14.94\%$ & 70.6\% \\
LightGBM final --- top-30   & $\mathbf{+31.25\%}$ & 59\% & 0.36 & $-15.41\%$ & 72.5\% \\
\bottomrule
\end{tabular}
\caption{Portfolio backtest results on the held-out test set. The final
model achieves higher cumulative excess at the cost of a slightly deeper
maximum drawdown ($-15.41\%$ vs $-14.94\%$): concentrating in 30 names
amplifies both upside and downside in any single 5-day window, which is
the expected trade-off of higher conviction portfolios.}
\end{table}

\textbf{Note on identical win rates.}
Both models have a win rate of 59\% (24 out of 41 windows). This is not
coincidental: the two models share the same retraining schedule (every
20 days), the same feature set universe, and highly correlated predictions.
The wins and losses predominantly fall on the same calendar windows,
driven by market-level conditions rather than model-specific predictions.
Win rate therefore does not differentiate the models; cumulative excess
return does ($+31.25\%$ vs $+23.18\%$).
...
\caption{Left: Cumulative excess return over CSI500 benchmark during the
test period. Right: Distribution of 5-day window excess returns.
The final model (orange) achieves higher cumulative excess while maintaining
a similar win-rate distribution to the baseline (blue).}
\end{figure}
...
Top-30 vs top-40 vs top-50 (val set)
  & Top-30: $+50\%$, top-40: $+43\%$, top-50: $+38\%$; top-30 selected on val \\
\bottomrule
\end{tabular}
